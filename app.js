// Modal elements
const btn = document.querySelector(".rules-btn"); // rules button
const crossBtn = document.querySelector(".cross"); // cross icon
const modal = document.querySelector(".modal"); // modal box
const backdrop = document.querySelector(".backdrop"); // dark background

// removing preload class after page loads
setTimeout(() => {
    document.body.classList.remove("preload");
});

// Game choices with winning conditions
const CHOICES = [
...
// all buttons and result sections
...
// opening modal when rules button clicked
...
// adding click event on all choice buttons
choiceBtn.forEach(button => {

    button.addEventListener("click", () => {

        // getting user selected choice
        const choiceName = button.dataset.choice;

        // finding matching object from CHOICES array
...
function displayResults(result) {
    ...
    // showing user selected choice
    resultDivs[0].innerHTML = `
        <div class="choice ${result[0].name}">
            ${svgMap[result[0].name]}
        </div>
    `;

    // showing ai choice after 1 second
    setTimeout(() => {

        resultDivs[1].innerHTML = `
            <div class="choice ${result[1].name}">
                ${svgMap[result[1].name]}
            </div>
        `;
...
function displayWinner(results) {
    ...
    setTimeout(() => {
        ...
        // adding result layout
        resultDiv.classList.add('show-winner');
...
playAgainBtn.addEventListener("click", () => {
    ...
    // clearing winner text
    resultText.innerHTML = "";

    // hiding result section
    resultWinner.classList.toggle("hidden");

    // removing winner layout
